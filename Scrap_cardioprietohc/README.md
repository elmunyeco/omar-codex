...
Variables:
- `BASE_URL`: origen del sitio, ej. `https://cardioprietohc.com/`.
- `LOGIN_PATH`: path del login (vacío si no requiere login).
- `PACIENTES_PATH`: path de la lista de pacientes (default `/index.php/pacientes/index`).
- `HISTORIAS_PATH`: path de la lista de historias (default `/index.php/historias/index`).
